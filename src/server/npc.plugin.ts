...
export function npcPlugin(options?: NpcPluginOptions): OpenCorePlugin {
  return {
    ...
    install(ctx) {
      ...
      const wireBridge = options?.connected
        ? new NpcWireBridgeServer({
            call(name: string, ...args: unknown[]) {
              const rpc = GLOBAL_CONTAINER.resolve(RpcAPI as unknown as never) as RpcApiType<'server'>
              return (rpc.call as (...a: unknown[]) => Promise<unknown>)(name, ...args)
            },
          })
        : undefined
      ...
      const transport =
        preferredAdapter === 'redm'
          ? new RedMNpcTransportServer({ wireBridge, chooseExecutorClient })
          : new FiveMNpcTransportServer({ wireBridge, chooseExecutorClient })
